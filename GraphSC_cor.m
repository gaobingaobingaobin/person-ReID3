function [B S stat] = GraphSC_cor(X, W, num_bases, alpha, beta, num_iters, Binit, pars)
...
while t < pars.num_iters
    ...
    if t ==1
        ...
        %% gaobin begin
        S= learn_coefficients_fast(B, X, alpha, pars.beta/pars.sigma*pars.noise_var,L);
        %% gaobin end
    else
        %         S= learn_coefficients(B, X, alpha, pars.beta/pars.sigma*pars.noise_var, L, S);
        %% gaobin added
        S= learn_coefficients_fast(B, X, alpha, pars.beta/pars.sigma*pars.noise_var,L,S);
        %% gaobin end
    end
    S(isnan(S))=0;
...
%     %% gaobin begin update Weight
%     W=GetW(S,5,2,1);% neightbor=15, distance type 2， lambda1=1
%     DCol = full(sum(W,2));
%     D = spdiags(DCol,0,speye(size(W,1)));
%     L = D - W;
%     %% gaobin end
